/*
 * wave Embedded JavaScript templates
...
/**
 * wave internal functions.
 *
 * Technically this "module" lies in the same file as {@link module:wave}, for
 * the sake of organization all the private functions re grouped into this
 * module.
 *
 * @module wave-internal
 * @private
 */
...
/**
 * wave template function cache. This can be a LRU object from lru-cache NPM
...
function handleCache(options, template) {
  ...
  if (options.cache) {
    ...
    if (!path) {
      throw new Error('Internal Template error: no file name or template '
                    + 'provided');
    }
    template = fs.readFileSync(path).toString().replace(_BOM, '');
  }
...
/**
 * Re-throw the given `err` in context to the `str` of wave, `filename`, and
 * `lineno`.
 *
 * @implements RethrowCallback
 * @memberof module:wave-internal
 * @param {Error}  err      Error object
 * @param {String} str      wave source
 * @param {String} filename file name of the wave file
 ...
 * options object.
 *
 * This is used for compatibility with earlier versions of wave and Express.js.
 *
 * @memberof module:wave-internal
...
/**
 * Compile the given `str` of wave into a template function.
 *
 * @param {String}  template wave template
 *
...
exports.compile=function compile(template, opts) {
  ...
  if (opts && opts.scope) {
    if (!scopeOptionWarned){
      console.warn('`scope` option is deprecated and will be removed in wave 3');
...
/**
 * Render the given `template` of wave.
 *
...
/**
 * Render an wave file at the given `path` and callback `cb(err, str)`.
 *
 * If you would like to include options but not data, you need to explicitly
 * call this function with `data` being an empty object or `null`.
 *
 * @param {String}             path     path to the wave file
 * @param {Object}            [data={}] template data
 * @param {Options}           [opts={}] compilation and rendering options
...
, compile: function () {
    ...
    try {
      ...
      if (e instanceof SyntaxError) {
        ...
        e.message += ' while compiling wave';
      }
      throw e;
    }
 ...
 * Express.js support.
 *
 * This is an alias for {@link module:wave.renderFile}, in order to support
...
if (require.extensions) {
  require.extensions['.wave'] = function (module, filename) {
